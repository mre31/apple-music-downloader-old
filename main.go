...
var (
	...
	currentDecryptPortIndex = 0
	decryptPortMutex    sync.Mutex // Şifre çözme port indeksi için mutex
	currentGetM3u8PortIndex = 0
	getM3u8PortMutex    sync.Mutex // M3U8 port indeksi için mutex
...
func downloadTrack(trackNum int, trackTotal int, meta *structs.AutoGenerated, track structs.TrackData, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec string, covPath string) {
	fmt.Printf("Attempting to download Track %d of %d: %s\n", trackNum, trackTotal, track.Attributes.Name)

	for currentRetry := 0; currentRetry < maxRetries; currentRetry++ {
		if currentRetry > 0 {
			fmt.Printf("Retrying Track %d (%s) - Attempt %d/%d after %v delay...\n", trackNum, track.Attributes.Name, currentRetry+1, maxRetries, retryDelay)
		...
		if track.Type == "music-videos" {
			if len(mediaUserToken) <= 50 {
				fmt.Printf("Track %d (%s): media-user-token is not set, skip MV dl. Retry %d/%d\n", trackNum, track.Attributes.Name, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Success++ // Sayacı yine de başarılı sayalım çünkü MV atlandı, hata değil.
				}
				// MV için token yoksa tekrar denemenin anlamı yok, direkt çıkabiliriz veya döngü sonunda success sayılır.
				// Bu örnekte, deneme sayısı sonunda success sayılacak.
				continue // Hata olarak işaretlemiyoruz, sadece atlıyoruz ve bir sonraki denemeye geçiyoruz (eğer varsa)
			}
			if _, err := exec.LookPath("mp4decrypt"); err != nil {
				fmt.Printf("Track %d (%s): mp4decrypt is not found, skip MV dl. Retry %d/%d\n", trackNum, track.Attributes.Name, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Success++ // mp4decrypt yoksa bu bir kurulum sorunudur, program hatası değil.
				}
				continue
			}
			err := mvDownloader(track.ID, sanAlbumFolder, token, storefront, mediaUserToken, meta)
			if err != nil {
				fmt.Printf("Track %d (%s): Failed to dl MV: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, err, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Error++
				}
				continue
			...
			counter.Success++
			fmt.Printf("Track %d (%s): MV Downloaded Successfully.\n", trackNum, track.Attributes.Name)
			return // MV başarılıysa fonksiyondan çık
		}

		manifest, err := getInfoFromAdam(track.ID, token, storefront)
		if err != nil {
			fmt.Printf("Track %d (%s): Failed to get manifest: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, err, currentRetry+1, maxRetries)
			if currentRetry == maxRetries-1 {
				counter.NotSong++
			}
			continue
		...
		if manifest.Attributes.ExtendedAssetUrls.EnhancedHls == "" {
			if dl_atmos {
				fmt.Printf("Track %d (%s): Atmos unavailable. Retry %d/%d\n", trackNum, track.Attributes.Name, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Unavailable++
				}
				continue
			}
			fmt.Printf("Track %d (%s): Manifest unavailable, Try DL AAC-LC. Retry %d/%d\n", trackNum, track.Attributes.Name, currentRetry+1, maxRetries)
			needDlAacLc = true
		}
		needCheck := false
		...
		if needCheck && !needDlAacLc {
			EnhancedHls_m3u8, err = checkM3u8(track.ID, "song") // err kontrolü eklendi
			if err != nil {
				fmt.Printf("Track %d (%s): Failed to checkM3u8: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, err, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Error++ // checkM3u8 hatası genel bir hata olarak sayılabilir.
				}
				continue
			}
		...
		if strings.Contains(Config.SongFileFormat, "Quality") {
			if dl_atmos {
				...
				if err != nil {
					fmt.Printf("Track %d (%s): Failed to extract quality from manifest: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, err, currentRetry+1, maxRetries)
					if currentRetry == maxRetries-1 {
						counter.Error++
					}
					continue
		...
		fmt.Printf("Track %d (%s): Filename will be: %s\n", trackNum, track.Attributes.Name, songName)
		...
		if Config.EmbedLrc || Config.SaveLrcFile {
			lrcStr, errLrc := lyrics.Get(storefront, track.ID, Config.LrcType, Config.Language, Config.LrcFormat, token, mediaUserToken)
			if errLrc != nil {
				fmt.Printf("Track %d (%s): Failed to get lyrics: %v. This is non-fatal.\n", trackNum, track.Attributes.Name, errLrc) // Lirik hatası önemli değil, devam et
			} else {
				if Config.SaveLrcFile {
					errWriteLrc := writeLyrics(sanAlbumFolder, lrcFilename, lrcStr)
					if errWriteLrc != nil {
						fmt.Printf("Track %d (%s): Failed to write lyrics: %v. This is non-fatal.\n", trackNum, track.Attributes.Name, errWriteLrc)
					}
				}
				if Config.EmbedLrc {
					lrc = lrcStr
		...
		exists, errFileExists := fileExists(trackPath)
		if errFileExists != nil {
			fmt.Printf("Track %d (%s): Failed to check if track exists: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, errFileExists, currentRetry+1, maxRetries)
			if currentRetry == maxRetries-1 {
				counter.Error++
			}
			continue
		...
		if exists {
			fmt.Printf("Track %d (%s): Already exists locally.\n", trackNum, track.Attributes.Name)
			counter.Success++
			okDict[albumId] = append(okDict[albumId], trackNum)
			return // Şarkı zaten varsa fonksiyondan çık
		}
		if needDlAacLc {
			if len(mediaUserToken) <= 50 {
				fmt.Printf("Track %d (%s): Invalid media-user-token for AAC-LC. Retry %d/%d\n", trackNum, track.Attributes.Name, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Error++
				}
				continue
			}
			_, errRunV3 := runv3.Run(track.ID, trackPath, token, mediaUserToken, false)
			if errRunV3 != nil {
				fmt.Printf("Track %d (%s): Failed to dl aac-lc: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, errRunV3, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Error++
				}
				continue
			...
			trackM3u8Url, _, errExtractMedia := extractMedia(manifest.Attributes.ExtendedAssetUrls.EnhancedHls, false)
			if errExtractMedia != nil {
				fmt.Printf("Track %d (%s): Failed to extract info from manifest: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, errExtractMedia, currentRetry+1, maxRetries)
				if currentRetry == maxRetries-1 {
					counter.Unavailable++
				}
				continue
			...
			if errRunV2 != nil {
				...
				// Koşulu "decryptFragment: EOF" hatasını da içerecek şekilde genişlet
				if strings.Contains(errRunV2.Error(), "connect: connection refused") || 
				   strings.Contains(errRunV2.Error(), "dial tcp") || 
				   strings.Contains(errRunV2.Error(), "decryptFragment: EOF") {
		...
		if Config.EmbedCover {
			if strings.Contains(albumId, "pl.") && Config.DlAlbumcoverForPlaylist {
				trackCovPath, errWriteCover := writeCover(sanAlbumFolder, track.ID, track.Attributes.Artwork.URL)
				if errWriteCover != nil {
					fmt.Printf("Track %d (%s): Failed to write cover: %v. This is non-fatal.\n", trackNum, track.Attributes.Name, errWriteCover)
				} else {
		...
		if errEmbed := cmd.Run(); errEmbed != nil {
			fmt.Printf("Track %d (%s): Embed failed: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, errEmbed, currentRetry+1, maxRetries)
			// MP4Box hatası genellikle dosyanın bozuk olduğu veya MP4Box'ın olmadığı anlamına gelir.
			// Yeniden denemek muhtemelen işe yaramaz, bu yüzden son denemede hata sayacını artıralım.
			if currentRetry == maxRetries-1 {
				counter.Error++
			}
			continue 
		...
		if strings.Contains(albumId, "pl.") && Config.DlAlbumcoverForPlaylist && trackCovPath != "" {
			if errRemoveCover := os.Remove(trackCovPath); errRemoveCover != nil {
				fmt.Printf("Track %d (%s): Error deleting temporary cover file: %s: %v. This is non-fatal.\n", trackNum, track.Attributes.Name, trackCovPath, errRemoveCover)
			}
		}
		errWriteMP4Tags := writeMP4Tags(trackPath, lrc, meta, trackNum, trackTotal)
		if errWriteMP4Tags != nil {
			fmt.Printf("Track %d (%s): Failed to write tags in media: %v. Retry %d/%d\n", trackNum, track.Attributes.Name, errWriteMP4Tags, currentRetry+1, maxRetries)
			if currentRetry == maxRetries-1 {
				counter.Unavailable++ // Etiketleme hatası genellikle bir 'unavailable' durumu gibi ele alınır.
			}
			continue
		}
		counter.Success++
		okDict[albumId] = append(okDict[albumId], trackNum)
		fmt.Printf("Track %d (%s): Successfully downloaded and processed.\n", trackNum, track.Attributes.Name)
		return // Başarılıysa fonksiyondan çık
	}

	// Eğer döngü maxRetries'e ulaştıysa ve hala return olmadıysa, bu şarkı için işlem başarısız olmuştur.
	// Sayaçlar zaten ilgili continue bloklarında son denemede artırıldığı için burada tekrar artırmaya gerek yok.
	fmt.Printf("Track %d (%s): Failed to process after %d retries.\n", trackNum, track.Attributes.Name, maxRetries)
}
...
func rip(albumId string, token string, storefront string, mediaUserToken string, urlArg_i string) error {
	...
	var wg sync.WaitGroup // WaitGroup tanımla
	numTracksToDownload := 0
	for _, trackNum := range selected {
		if !isInArray(okDict[albumId], trackNum) {
			numTracksToDownload++
		}
	}
	wg.Add(numTracksToDownload) // İndirilecek şarkı sayısı kadar sayacı başlat

	for trackIndex, track := range meta.Data[0].Relationships.Tracks.Data {
		trackNum := trackIndex + 1 // trackNum 1-bazlı
		if isInArray(okDict[albumId], trackNum) {
			//fmt.Println("已完成直接跳过.\n")
			counter.Success++
			continue
		...
		if isInArray(selected, trackNum) {
			downloadSemaphore <- struct{}{} // Semaforu al
			counter.Total++ // Semafor alındıktan sonra toplam sayacı artır
			go func(tn int, tr structs.TrackData) { // Her şarkı için bir goroutine başlat
				defer wg.Done() // Goroutine tamamlandığında lokal sayacı azalt
				defer func() { <-downloadSemaphore }() // Semaforu bırak
				downloadTrack(tn, trackTotal, meta, tr, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec, covPath)
			}(trackNum, track)
		}
	}
	wg.Wait() // Tüm indirme goroutine'lerinin (bu albüm için) tamamlanmasını bekle
	return nil
}
...
func main() {
	...
	inputFile = pflag.StringP("file", "f", "", "File path to read URLs from (one URL per line)") // Yeni flag tanımı
	...
	// Semaforu başlat
	numDecryptPorts := Config.DecryptM3u8PortEnd - Config.DecryptM3u8PortStart + 1
	maxConcurrentDownloads := numDecryptPorts
	if maxConcurrentDownloads <= 0 {
		maxConcurrentDownloads = 1 // En az 1 olmalı
	}
	if maxConcurrentDownloads > 50 { // Güvenlik için üst sınır
		maxConcurrentDownloads = 50
	}
	downloadSemaphore = make(chan struct{}, maxConcurrentDownloads)
	fmt.Printf("Concurrency level set to: %d\n", maxConcurrentDownloads)

	args := pflag.Args()

	// Dosyadan URL'leri oku (eğer -file argümanı verilmişse)
	if *inputFile != "" {
		...
		for scanner.Scan() {
			...
			if line != "" && !strings.HasPrefix(line, "#") { // Boş satırları ve yorumları atla
				urlsFromFile = append(urlsFromFile, line)
			}
		}
		...
		args = append(args, urlsFromFile...) // Dosyadaki URL'leri mevcut argümanlara ekle
	}

	if len(args) == 0 {
		fmt.Println("No URLs provided directly or via file. Please provide at least one URL or use the -f flag.")
		pflag.Usage()
		return
	}
	os.Args = args
	...
	for { // Bu sonsuz döngü, hata durumunda yeniden deneme mantığını koruyor
		for albumNum, urlRaw := range os.Args {
			globalWg.Add(1) // Her albüm/URL işlemi için global WaitGroup'a ekle
			go func(currentUrl string, currentAlbumNum int) { // Her URL için bir goroutine başlat
				defer globalWg.Done() // Goroutine bittiğinde global WaitGroup'tan çıkar
				fmt.Printf("Processing Album %d of %d: %s\n", currentAlbumNum+1, albumTotal, currentUrl)
				var storefront, albumId string
				//mv dl dev
				if strings.Contains(currentUrl, "/music-video/") {
					if debug_mode {
						return // Goroutine'dan çık
					}
					downloadSemaphore <- struct{}{} // Semafor al
					defer func() { <-downloadSemaphore }() // Semaforu bırak
					counter.Total++
					...
					storefront, albumId = checkUrlMv(currentUrl)
				...
				var originalDlSongState = dl_song // Orijinal dl_song durumunu sakla
				if strings.Contains(currentUrl, "/song/") {
					var err_song_url error
					currentUrl, err_song_url = getUrlSong(currentUrl, token)
					dl_song = true // Bu goroutine için dl_song'u etkinleştir
					if err_song_url != nil {
						fmt.Println("Failed to get Song info for", currentUrl, ":", err_song_url)
						counter.Error++ // Hata sayacını artır
						return // Goroutine'dan çık
					}
				}
				if strings.Contains(currentUrl, "/playlist/") {
					storefront, albumId = checkUrlPlaylist(currentUrl)
				} else {
					storefront, albumId = checkUrl(currentUrl)
				}
				if albumId == "" {
				...
				parse, err_parse := url.Parse(currentUrl)
				if err_parse != nil {
					log.Printf("Invalid URL for parsing query param: %v for %s", err_parse, currentUrl)
					counter.Error++ // Hata sayacını artır
					return // Goroutine'dan çık
				}
				var urlArg_i = parse.Query().Get("i")
				err_rip := rip(albumId, token, storefront, Config.MediaUserToken, urlArg_i)
				dl_song = originalDlSongState // dl_song durumunu eski haline getir
				if err_rip != nil {
					fmt.Println("Album failed for", currentUrl, ":", err_rip)
					counter.Error++ // Hata sayacını artır
				}
			}(urlRaw, albumNum)
		}
		globalWg.Wait() // Tüm albüm/URL işlemleri goroutine'lerinin tamamlanmasını bekle
