...
func downloadTrack(trackNum int, trackTotal int, meta *structs.AutoGenerated, track structs.TrackData, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec string, covPath string) {
	counter.Total++ // Şarkı denemeye başlandığında toplam sayacı bir kez artır
	...
	for trackIndex, track := range meta.Data[0].Relationships.Tracks.Data {
		...
		if isInArray(selected, trackNum) {
			go func(tn int, tr structs.TrackData) { // Her şarkı için bir goroutine başlat
				defer wg.Done() // Goroutine tamamlandığında sayacı azalt
				downloadTrack(tn, trackTotal, meta, tr, albumId, token, storefront, mediaUserToken, sanAlbumFolder, Codec, covPath)
			}(trackNum, track)
		}
	}
	wg.Wait() // Tüm indirme goroutine'lerinin tamamlanmasını bekle
	return nil
}
